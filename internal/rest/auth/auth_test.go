...
func TestCheckIsAuthorized(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectedStatus: http.StatusOK,
			expectedBody:   `{"body":{"isAuthorized":true}}`,
			setupMocks: func(fields *fields) {
				fields.SessionStorage.EXPECT().GetUserIDBySession(gomock.Any(), "valid_session_id").Return(uint(1), nil)
			},
		},
		{
			name: "no session",
			...
			expectedStatus: http.StatusOK,
			expectedBody:   `{"body":{"isAuthorized":false}}`,
			setupMocks: func(fields *fields) {
			},
		},
		{
			...
			expectedBody:   `{"body":{"isAuthorized":false}}`,
			setupMocks: func(fields *fields) {
				fields.SessionStorage.EXPECT().GetUserIDBySession(gomock.Any(), "valid_session_id").Return(uint(1), errors.ErrNotFound)
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			rr := httptest.NewRecorder()
			handler.CheckIsAuthorized(rr, req)
